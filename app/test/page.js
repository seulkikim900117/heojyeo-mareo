...
const YEARS = Array.from({ length: 80 }, (_, i) => thisYear - 15 - i); // 15세~94세
...
{DAYS.map((d) => (
  ...
  {d}일
      ...
      <select value={value.hour} onChange={(e) => set("hour", e.target.value)}>
          <option value="">몰라요 (시주 제외하고 계산)</option>
{HOURS.map((h) => (
            <option key={h} value={h}>
  {String(h).padStart(2, "0")}시 ~ {String(h).padStart(2, "0")}시 59분
  </option>
        ))}
          </select>
          </div>
...
const empty = { name: "", year: "", month: "", day: "", hour: "" };
