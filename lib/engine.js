...
const core = {
목: yang ? "곶게 끩는 나무처럼 방향이 분명하고 한번 정하면 밀고 나가는 성향" : "덩굴처럼 유연하게 감고 올라가 어떤 환경에도 적응하는 성향",
...
const cautions = [];
if (group.관성 === 0) cautions.push("관성이 없어 스스로 규율을 세우지 않으면 흐트러지기 쉽워요.");
...
if (isSixHarmony(ab, bb)) { score += 16; notes.push("일지 " + ad.branch + "·" + bd.branch + " 육합 — 배우자 자리가 딱 맞물리는 최고 조합 중 하나예요."); }
...
if (t) { score += 9; notes.push("띠 삼합 — " + a.zodiac + "띠와 " + b.zodiac + "띠는 전통적으로 최고로 치는 조합이에요."); }
else if (isSixHarmony(ay, by)) { score += 8; notes.push("띠 육합 — " + a.zodiac + "띠와 " + b.zodiac + "띠는 서로 부족한 걸 메워주는 관계예요."); }
else if (isClash(ay, by)) { score -= 7; notes.push("띠 충 — " + a.zodiac + "띠와 " + b.zodiac + "띠는 처음엔 강하게 끌리지만 오래 부딪히기도 해요."); }
...
: score >= 60 ? ["🙌", "좀 더 봐봐", "나쁘지 않아요. 다만 서로 맞추는 구간이 필요해요."]
