...
sections.push(crossSection(a, b));
...
const SCALE = [[26, 20], [48, 47], [53, 57], [58, 67], [65, 77], [82, 89], [100, 99]];
let disp = 99;
...
// ---------- 교차 분석: 두 사람 8글자 전체 대조 ----------
const POSN = ["년", "월", "일", "시"];

function pillarList(r) {
return [r.pillars.year, r.pillars.month, r.pillars.day, r.pillars.hour];
}

export function crossLinks(a, b) {
const A = nameOf(a), B = nameOf(b);
const pa = pillarList(a), pb = pillarList(b);
const links = [];
...
pb.forEach((y, j) => {
if (!y) return;
const u = x.branchIdx, v = y.branchIdx;
const tag = A + " " + POSN[i] + "지 " + x.branch + " × " + B + " " + POSN[j] + "지 " + y.branch;
if (isSixHarmony(u, v)) links.push({ kind: "육합", good: true, text: tag + " → 육합" });
const t = triadOf(u, v);
if (t) links.push({ kind: "반합", good: true, text: tag + " → " + ELEMENTS[t[3]] + "국 반합" });
if (isClash(u, v)) links.push({ kind: "충", good: false, text: tag + " → 충", au: u, bu: v, ai: i, bj: j });
if (isPo(u, v)) links.push({ kind: "파", good: false, text: tag + " → 파(破)" });
if (isWonjin(u, v)) links.push({ kind: "원진", good: false, text: tag + " → 원진" });
});
});
...
pb.forEach((y, j) => {
if (!y) return;
if ((x.stemIdx - y.stemIdx + 10) % 10 === 5) {
links.push({ kind: "천간합", good: true, text: A + " " + POSN[i] + "주 " + x.stem + " × " + B + " " + POSN[j] + "주 " + y.stem + " → 천간합(" + STEMS_CN[x.stemIdx] + STEMS_CN[y.stemIdx] + ")" });
}
});
});

const pool = [];
pa.forEach((p, i) => { if (p) pool.push({ b: p.branchIdx, who: A, pos: POSN[i] }); });
pb.forEach((p, i) => { if (p) pool.push({ b: p.branchIdx, who: B, pos: POSN[i] }); });
const unions = [];
TRIADS.forEach((t) => {
const need = t.slice(0, 3);
const parts = need.map((n) => pool.find((x) => x.b === n));
if (!parts.every(Boolean)) return;
const owners = {};
parts.forEach((p) => { owners[p.who] = true; });
if (Object.keys(owners).length < 2) return;
const el = ELEMENTS[t[3]];
const eIdx = t[3];
const fills = [];
[a, b].forEach((r) => {
const nm = nameOf(r);
const cnt = r.elements.count[eIdx];
const gm = need.some((n) => r.gongmang.branches.indexOf(BRANCHES[n]) >= 0);
if (cnt === 0) fills.push({ who: nm, mode: "empty", gongmang: gm });
else if (gm) fills.push({ who: nm, mode: "gongmang", gongmang: true });
});
unions.push({
element: el,
name: need.map((n) => BRANCHES[n]).join("") + " " + el + "국",
parts: parts.map((p) => BRANCHES[p.b] + "(" + p.who + " " + p.pos + ")"),
fills: fills,
});
});

const opens = [];
links.filter((l) => l.kind === "충").forEach((l) => {
[[b, l.bu, l.bj, B], [a, l.au, l.ai, A]].forEach((pair) => {
const r = pair[0], bi = pair[1], pi = pair[2], nm = pair[3];
const spouse = r.input.gender === "F" ? ["편관", "정관"] : r.input.gender === "M" ? ["편재", "정재"] : [];
HIDDEN[bi].forEach((h) => {
const st = h[0];
const g = tenGod(r.pillars.day.stemIdx, st);
const isSpouse = spouse.indexOf(g) >= 0;
const lacking = r.elements.count[STEM_ELEMENT[st]] === 0;
if (isSpouse || lacking) {
opens.push({ who: nm, branch: BRANCHES[bi], pos: POSN[pi], stem: STEMS[st], god: g, spouse: isSpouse, lacking: lacking });
}
});
});
});

return { links: links, unions: unions, opens: opens };
}

export function crossSection(a, b) {
const c = crossLinks(a, b);
const L = [];
const good = c.links.filter((x) => x.good);
const bad = c.links.filter((x) => !x.good);

L.push("같은 자리끼리가 아니라 두 사람의 여덟 글자를 전부 맞대어 본 결과예요.");

if (good.length) {
L.push("");
L.push("[ 서로 끌어당기는 자리 ]");
good.forEach((g) => L.push("· " + g.text));
}
if (bad.length) {
L.push("");
L.push("[ 부딪히는 자리 ]");
bad.forEach((g) => L.push("· " + g.text));
}

if (c.unions.length) {
L.push("");
L.push("[ 둘이 합쳐야 완성되는 국(局) ]");
c.unions.forEach((u) => {
L.push("· " + u.parts.join(" + ") + " = " + u.name);
if (u.fills.length) {
u.fills.forEach((f) => {
if (f.mode === "empty") {
L.push("  → " + f.who + "에게 " + josa(u.element, "은", "는") + " 원래 한 글자도 없던 오행이에요. 혼자서는 절대 못 만드는 기운을 상대가 들고 옵니다." + (f.gongmang ? " 게다가 공망까지 걸려 있던 자리라 더 그렇습니다." : ""));
} else {
L.push("  → " + f.who + " 기준으로 이 국에 들어가는 지지 중 하나가 공망 자리예요. 비어 있던 칸이 상대 덕분에 채워집니다.");
}
});
} else {
L.push("  → 둘이 함께 있을 때만 만들어지는 " + u.element + " 기운이에요.");
}
});
}
...
L.push("");
L.push("[ 충이 여는 문 ]");
L.push("충은 보통 나쁘게 보지만, 닫힌 지지를 열어 그 안의 지장간을 꺼내는 열쇠이기도 해요.");
c.opens.forEach((o) => {
let s = "· " + o.who + "의 " + o.pos + "지 " + josa(o.branch, "이", "가") + " 열리면 그 안에 있던 " + o.stem + "(" + o.god + ") 기운이 드러나요.";
if (o.spouse) s += " 숨어 있던 배우자 기운이 상대 때문에 꺼내지는 구조입니다.";
else if (o.lacking) s += " 사주에 아예 없던 오행이 이때 잠깐 살아나요.";
L.push(s);
});
}

if (!good.length && !bad.length && !c.unions.length) {
L.push("");
L.push("여덟 글자를 전부 대조했지만 두드러진 교차 관계가 없어요. 사주가 서로에게 크게 간섭하지 않는 조합이라, 관계의 결과는 두 분이 만들어가는 쪽에 훨씬 크게 달려 있습니다.");
}

return { title: "🔗 둘 사이의 연결 (8글자 교차)", text: L.join("\n") };
}
