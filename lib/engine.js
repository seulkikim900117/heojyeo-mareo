...
score = Math.round(Math.max(20, Math.min(99, 68 + (score - 50) * 0.62)));

const v = score >= 88 ? ["💞", "절대 말어!!", "이 조합 놓치면 후회해요. 사주가 먼저 밀어주는 사이예요."]
: score >= 73 ? ["💕", "말어~", "웬만하면 계속 만나세요. 이 정도 궁합 흔치 않아요."]
: score >= 65 ? ["🙌", "좀 더 봐봐", "나쁘지 않아요. 다만 서로 맞추는 구간이 필요해요."]
: score >= 58 ? ["😐", "반반", "잘 되면 오래가고, 안 되면 길게 힘들어요. 대화가 전부예요."]
: score >= 50 ? ["😵", "음... 글쎄", "기질 차이가 커요. 서로 바꾸려 들면 더 힘들어져요."]
