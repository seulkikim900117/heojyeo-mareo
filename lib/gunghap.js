...
export function analyzeGunghap(person1, person2) {
  ...
  const b1 = s1.pillars.day.branch, b2 = s2.pillars.day.branch;
        ...
        if (branchYukhap(b1, b2)) {
                  ...
                  spouseText = `${BRANCHES[b1]}와 ${BRANCHES[b2]}는 귀문관살 관계예요. 좋을 땐 말 안 해도 통하는 텔레파시급 케미인데, 틀어지면 상대의 사소한 것까지 신경 쓰이고 집착하게 되는 조합입니다. 감정의 진폭이 큰 만큼 서로의 예민함을 인정해 주는 게 핵심이에요.`;
        ...
        if (g1 && g2) {
                  ...
                  for (const [name, self, gender, other, otherName] of pairs) {
                              ...
                              if (own === 0 && fromPartner >= 2) {
                                            ...
                                            parts.push(
                                                            `${name}의 사주엔 배우자 기운인 ${starName}(${ELEMENTS[E]}${ELEMENT_EMOJI[E]})이 ${own}개로 아주 강해요. 배우자 자리가 뚜렷한, 결혼 인연이 잘 드러나는 사주입니다.`
                                                          );
                              } else {
                                            score += 1;
                                            parts.push(
