...
export function analyzeGunghap(person1, person2) {
  ...
  const d1 = s1.dayMaster, d2 = s2.dayMaster;
    ...
    if (stemHap(d1, d2)) {
          ...
          mindText = `${n1}의 일간 ${STEMS[d1]}(${ELEMENTS[e1]})와 ${n2}의 일간 ${STEMS[d2]}(${ELEMENTS[e2]})는 천간합! 사주에서 손꼽히는 인연으로, 처음 만났을 때부터 뭔가 통한다는 느낌을 받았을 조합이에요. 서로가 서로에게 자석처럼 끓립니다.`;
    } else {
  ...
  const b1 = s1.pillars.day.branch, b2 = s2.pillars.day.branch;
    ...
    if (branchYukhap(b1, b2)) {
          ...
          spouseText = `${BRANCHES[b1]}와 ${BRANCHES[b2]}는 원진 관계예요. 평소엔 좋다가도 어느 순간 상대의 사소한 행동이 이유 없이 거슬리는 날이 옵니다. "욄 숨 쉬는 것도 얀미워" 모드가 주기적으로 온다면 원진의 소행이에요.`;
  ...
  const y1 = s1.pillars.year.branch, y2 = s2.pillars.year.branch;
    ...
    if (branchYukhap(y1, y2)) {
          score += 7;
          goods.push(`띄 육합 — ${s1.zodiac}띄·${s2.zodiac}띄 찰떡 조합`);
          zodiacText = `${s1.zodiac}띄와 ${s2.zodiac}띄는 육합이에요. 어르신들이 "궁합 좋다" 할 때 보는 게 바로 이 띄 궁합인데, 두 분은 합격입니다. 집안 어른들 반응도 좋을 확률이 높아요.`;
    } else if (branchSamhap(y1, y2)) {
          score += 6;
          goods.push(`띄 삼합 — ${s1.zodiac}띄·${s2.zodiac}띄 환상의 팀`);
          zodiacText = `${s1.zodiac}띄와 ${s2.zodiac}띄는 삼합! 전통적으로 최고로 치는 띄 조합 중 하나예요. 서로의 부족한 부분을 자연스럽게 메워주는 관계입니다.`;
    } else if (branchChung(y1, y2)) {
          score -= 8;
          bads.push(`띄 충 — ${s1.zodiac}띄·${s2.zodiac}띄는 상충 관계`);
          zodiacText = `${s1.zodiac}띄와 ${s2.zodiac}띄는 충이에요. 어른들이 걱정할 수 있는 조합이지만, 띄 궁합은 사주 전체에서 비중이 크지 않아요. 일간·일지 궁합이 좋다면 크게 신경 쓸 필요 없습니다.`;
    } else if (branchWonjin(y1, y2)) {
          score -= 5;
          bads.push(`띄 원진 — ${s1.zodiac}띄·${s2.zodiac}띄`);
          zodiacText = `${s1.zodiac}띄와 ${s2.zodiac}띄는 원진 관계예요. 서로 스타일이 달라 답답할 때가 있지만, 그만큼 서로에게 없는 걸 갖고 있다는 뜻이기도 해요.`;
    } else {
          zodiacText = `${s1.zodiac}띄와 ${s2.zodiac}띄 — 특별한 합충 없이 무난한 띄 조합이에요.`;
    }
  ...
  let fillText = "";
    ...
    if (fillCount > 0) {
          ...
          fillText = fills.join(". ") + ". 사주에서 나에게 없는 오행을 가진 사람에게 끓리는 건 본능이에요. 같이 있으면 이상하게 안정되는 이유가 여기 있습니다.";
    } else {
  ...
  let verdict, verdictEmoji, verdictSub;
    if (score >= 85) {
          ...
          verdict = "말어~";
          verdictEmoji = "💕";
          verdictSub = "웠만하면 계속 만나세요. 이 정도 궁합 흔치 않아요.";
    } else if (score >= 55) {
          verdict = "음... 말어?";
          verdictEmoji = "🤔";
