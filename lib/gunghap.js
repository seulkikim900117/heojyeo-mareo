...
// 전통 궁합 이론(천간합, 지지 육합/삼합/충/형/원진/귀문/파,
// 오행 상생상극, 십성 배우자성)을 재미 위주로 점수화한 것.
// ============================================================
...
const HYEONG_PAIRS = [[2, 5], [5, 8], [2, 8], [1, 10], [10, 7], [1, 7], [0, 3]];
...
const branchGwimun = (a, b) => GWIMUN_PAIRS.some(([x, y]) => (a === x && b === y) || (a === y && b === x));
const PA_PAIRS = [[0, 9], [1, 4], [2, 11], [3, 6], [5, 8], [7, 10]];
const branchPa = (a, b) => PA_PAIRS.some(([x, y]) => (a === x && b === y) || (a === y && b === x));

const spouseStarElement = (dayMasterElement, gender) =>
        gender === "M" ? (dayMasterElement + 2) % 5 : (dayMasterElement + 3) % 5;
...
export function analyzeGunghap(person1, person2) {
  ...
  const b1 = s1.pillars.day.branch, b2 = s2.pillars.day.branch;
        ...
        } else if (branchGwimun(b1, b2)) {
                  score -= 7;
                  bads.push("일지 귀문(鬼門) — 예민하게 얽히는 조합");
                  spouseText = `${BRANCHES[b1]}와 ${BRANCHES[b2]}는 귀문관살 관계예요. 좋을 때진 말 안 해도 통하는 텔레파시급 케미인데, 틀어지면 상대의 사소한 것까지 신경 쓰이고 집착하게 되는 조합입니다. 감정의 진폭이 큰 만큼 서로의 예민함을 인정해 주는 게 핵심이에요.`;
        } else if (branchPa(b1, b2)) {
                  score -= 6;
                  bads.push("일지 파(破) — 잔잔한 균열이 생기기 쉬운 조합");
                  spouseText = `${BRANCHES[b1]}와 ${BRANCHES[b2]}는 파(破) 관계예요. 큰 싸움은 아닌데 소소한 마찰과 어긋남이 반복되기 쉬운 구조입니다. 자잘한 서운함을 쌓아두지 말고 그때그때 풀어주면 충분히 커버되는 수준이에요.`;
        } else {
                  score += 2;
  ...
  // 4) 배우자성 분석 (십성: 재성·관성)
  let spouseStarText = "";
        const g1 = person1.gender, g2 = person2.gender;
        if (g1 && g2) {
                  const pairs = [
                              [n1, s1, g1, s2, n2],
                              [n2, s2, g2, s1, n1],
                  ...
                  const parts = [];
                  for (const [name, self, gender, other, otherName] of pairs) {
                              const E = spouseStarElement(self.dayMasterElement, gender);
                              const starName = gender === "M" ? "재성" : "관성";
                              const own = self.elements[E];
                              const fromPartner = other.elements[E];
                              if (own === 0 && fromPartner >= 2) {
                                            score += 9;
                                            goods.push(`${name}의 배우자성(${ELEMENTS[E]})을 상대가 채워주는 인연`);
                                            parts.push(
                                                            `${name}의 사주에는 배우자 기운인 ${starName}(${ELEMENTS[E]}${ELEMENT_EMOJI[E]})이 겉으로 드러나지 않아요. 그런데 ${otherName}의 사주에 ${iGa(ELEMENTS[E])} ${fromPartner}개나 있어서, 본인에게 부족한 배우자 인연을 상대가 통째로 채워줍니다. 사주가 말하는 "만나야 할 사람" 유형이에요.`
                                                          );
                              } else if (own === 0 && fromPartner === 0) {
                                            score -= 4;
                                            parts.push(
                                                            `${name}의 배우자 기운인 ${starName}(${ELEMENTS[E]})이 본인에게도 상대에게도 잘 드러나지 않아요. 인연을 오래 유지하려면 마음을 표현하는 의식적인 노력이 필요한 조합입니다.`
                                                          );
                              } else if (own >= 3) {
                                            score += 4;
                                            goods.push(`${name}의 배우자 자리(${starName})가 튼튼한 사주`);
                                            parts.push(
                                                            `${name}의 사주엔 배우자 기운인 ${starName}(${ELEMENTS[E]}${ELEMENT_EMOJI[E]})이 ${own}개로 아주 강해요. 배우자 자리가 뚚렷한, 결혼 인연이 잘 드러나는 사주입니다.`
                                                          );
                              } else {
                                            score += 1;
                                            parts.push(
                                                            `${name}의 배우자 기운인 ${starName}(${ELEMENTS[E]})은 ${own}개로 적당히 자리 잡고 있어요.`
                                                          );
                              }
                  }
                  spouseStarText = parts.join(" ");
        } else {
                  spouseStarText = "성별 정보가 있으면 재성·관성 기반의 배우자 인연 분석을 볼 수 있어요. 입력 화면에서 성별을 선택해 보세요.";
        }
  ...
  let fillText = "";
        ...
        for (let el = 0; el < 5; el++) {
                  if (s1.elements[el] === 0 && s2.elements[el] >= 2) {
                              fills.push(`${n2}의 넘치는 ${ELEMENTS[el]}${ELEMENT_EMOJI[el]} 기운이 ${n1}에게 없는 ${eulReul(ELEMENTS[el])} 채워줘요`);
                              fillCount++;
                  }
                  if (s2.elements[el] === 0 && s1.elements[el] >= 2) {
                              fills.push(`${n1}의 넘치는 ${ELEMENTS[el]}${ELEMENT_EMOJI[el]} 기운이 ${n2}에게 없는 ${eulReul(ELEMENTS[el])} 채워줘요`);
                              fillCount++;
                  }
        }
        if (fillCount > 0) {
  ...
  return {
            ...
            sections: [
                  ...
                  { title: "💍 배우자 인연 (재성·관성)", text: spouseStarText },
